...
namespace Test1
{
    public partial class SelectTrainer : Form
    {
        ...
        private void btn_select_Click(object sender, EventArgs e)
        {
            ...
            if (cmb_Trainers.SelectedItem != null && !string.IsNullOrWhiteSpace(txt_uname.Text))
            {
                // Get the selected item from the ComboBox and the value from the TextBox
                string selectedItem = cmb_Trainers.SelectedItem.ToString();
                ...
                try
                {
                    using (SqlConnection connection = new SqlConnection("Data Source=DESKTOP-NK2RF8T;Initial Catalog=PowerFit;Integrated Security=True"))
                    {
                        ...
                        string query = "INSERT INTO SelectTrainers(TrainerName,Usename) VALUES (@SelectedItem, @TextBoxValue)";
        ...
        private bool IsUsernameExists(string username)
        {
            using (SqlConnection connection = new SqlConnection("Data Source=DESKTOP-NK2RF8T;Initial Catalog=PowerFit;Integrated Security=True"))
            {
                try
                {
                    ...
                    // Prepare the SQL query with parameterized query to prevent SQL injection
                    string query = "SELECT COUNT(*) FROM SelectTrainers WHERE Usename = @Username";
                    SqlCommand command = new SqlCommand(query, connection);
            ...
            if (cmb_Trainers.SelectedItem != null && !string.IsNullOrWhiteSpace(txt_uname.Text))
            {
                ...
                try
                {
                    using (SqlConnection connection = new SqlConnection("Data Source=DESKTOP-NK2RF8T;Initial Catalog=PowerFit;Integrated Security=True"))
                    {
                        ...
                        // Prepare the SQL query with parameterized query to prevent SQL injection
                        string query = "UPDATE SelectTrainers SET TrainerName = @SelectedItem WHERE Usename = @TextBoxValue";
                        SqlCommand command = new SqlCommand(query, connection);
                        ...
                        if (rowsAffected > 0)
                        {
                            MessageBox.Show("Trainer Name updated in the database successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                            MessageBox.Show("You Have not Selected a trainer earlier.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
                catch (SqlException)
